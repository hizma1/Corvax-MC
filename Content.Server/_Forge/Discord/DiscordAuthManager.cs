...
public sealed partial class DiscordAuthManager : IPostInjectInit
{
    ...
    {
        _sponsors.RemoveSponsor(e.Channel.UserId);
    }
    ...
    private async Task<DiscordData> IsVerified(NetUserId userId, CancellationToken cancel = default)
    {
        ...
        try
        {
            ...
            {
                _sponsors.SetSponsor(userId, level);
                if (_playerMgr.TryGetSessionById(userId, out var session))
                {
                    ...
                    _sawmill.Info($"{userId} is sponsor now. UserId: {userId}. Level: {Enum.GetName(level)}:{(int) level}");
                }
            }
    ...
    private async Task<string?> GenerateLink(NetUserId userId, CancellationToken cancel = default)
    {
        ...
        try
        {
            ...
            {
                var retryAfter = response.Headers.RetryAfter?.Delta?.TotalSeconds ?? 30;
                await Task.Delay(TimeSpan.FromSeconds(retryAfter), cancel);
                return await GenerateLink(userId, cancel);
            }
            if (!response.IsSuccessStatusCode)
                return null;
    ...
    private Task<byte[]?> GenerateQrCode(string url)
    {
        try
        {
            ...
            byte[] darkColor = [255, 255, 255, 100];
            byte[] lightColor = [255, 255, 255, 0];
