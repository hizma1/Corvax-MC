rmc-no-queen-hivemind-chat = Королевы нет. Мы одни.
rmc-new-queen = Новая Королева возглавила Улей! Возрадуемся!
rmc-queen-death-cooldown-over = The Hive is ready for a new Queen to evolve. The hive can only survive for a limited time without a queen!
rmc-hive-core-cooldown-over = The Hive has recovered from the destruction of its previous core. A new Hive Core can now be placed.
rmc-hive-supports-castes = Теперь Улей может поддерживать: { $castes }
rmc-hive-supports-castes-human = где-то далеко вы слышите крик и чувствуете, как внутри всё замерло... что-то появилось в этой колонии.
rmc-hive-supports-castes-synth = Вы слышите далёкий крик неизвестной биоформы, похоже, она сообщает другим о смене иди эволюции формы. Вы начинаете анализировать и расшифровывать странную вокализацию.
